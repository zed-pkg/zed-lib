...
fn check_resolution(case: &Case, file: &str) {
    ...
    let requirement = case
        .requirement
        .as_deref()
        .unwrap_or_else(|| panic!("{file}:{}: resolution cases need a `requirement`", case.name));
    match (&case.expect.version, &case.expect.error) {
        (Some(want), None) => {
            let got = resolve_version(&meta, requirement)
                .unwrap_or_else(|error| panic!("{file}:{}: expected {want}, got {error}", case.name));
        ...
        _ => panic!("{file}:{}: declare exactly one of `version` or `error`", case.name),
    }
}
...
fn every_corpus_file_passes() {
    ...
    assert!(total > 100, "expected the generated corpus too, ran only {total}");
