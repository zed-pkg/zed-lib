...
const VERSION_POOL: &[&str] = &[
    "0.0.1", "0.1.0", "0.1.9", "0.2.0", "1.0.0", "1.0.1", "1.2.0", "1.2.3", "1.2.9", "1.3.0",
    "1.9.9", "2.0.0", "2.1.0", "10.0.0", "1.0.0-rc.1", "2.0.0-alpha.1", "1.2.3-beta.2",
    "v1.0.0", "v2.0.0+incompatible", "2026.01.01", "2026.06.01", "2026.07.24", "2026.10.01",
    "1.2.3rc1", "1.2.3.post1", "legacy-api", "release-candidate-1", "nightly",
];

const REQUIREMENT_POOL: &[&str] = &[
    "^1", "^1.2", "^1.2.3", "^0.1", "^0.0.1", "^2", "^2026", "~1", "~1.2", "~1.2.3",
    ">=1.2", ">=1.2 <2", ">1.0.0", "<2.0.0", "<=1.2.3", "=1.0.0", "=1.2", "*", "1", "1.2",
    "1.2.3", "1.*", "1.2.*", "1.x", "2026.07.24", ">=2026.7", "legacy-api", "nightly",
    "^1.x.y", "not-a-version", ">= 1.0.0, < 2.0.0",
];
...
fn main() {
    ...
    for index in 0..400 {
        ...
        let expect = match resolve_version(&meta, requirement) {
            Ok(version) => format!("{{ \"version\": {} }}", serde_json::to_string(version).unwrap()),
        ...
        if index % 2 == 0 {
            let expect = match latest_stable(&meta) {
                Some(version) => {
                    format!("{{ \"version\": {} }}", serde_json::to_string(version).unwrap())
                }
